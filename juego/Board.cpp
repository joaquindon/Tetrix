...
// Aquí llamamos a NASM
void Board::Right()
{
    MoveRight(&board[0][0], &indX);
}

void Board::Left()
{
    MoveLeft(&board[0][0], &indX);
}
...
Board::Board() 
{
    ...
    // Correción: inicializar variables importantes
    timer = 0;
    indX = 0;
    indY = 0;
    indNewPart = 0;
    indColorNewPart = 1;
    newPartColor = sf::Color(50, 50, 50);
...
{
    bool limit = UpdateBoardASM(&board[0][0], &timer, timerLimit, &indY, indColorNewPart);
    timer++;
    return limit;
}
